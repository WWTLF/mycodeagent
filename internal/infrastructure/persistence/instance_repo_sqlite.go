...
func (r *SQLiteInstanceRepository) Save(inst *entity.Instance) error {
	result, err := r.db.Exec(
		`INSERT INTO instances (vastai_id, model_name, status, local_port, ssh_host, ssh_port, tunnel_pid, hourly_rate)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		inst.VastaiID, inst.ModelName, inst.Status, inst.LocalPort,
		inst.SSHHost, inst.SSHPort, inst.TunnelPID, inst.HourlyRate,
	)
	if err != nil {
		return err
	}
...
func (r *SQLiteInstanceRepository) Update(inst *entity.Instance) error {
	_, err := r.db.Exec(
		`UPDATE instances SET vastai_id=?, model_name=?, status=?, local_port=?,
		 ssh_host=?, ssh_port=?, tunnel_pid=?, hourly_rate=? WHERE id=?`,
		inst.VastaiID, inst.ModelName, inst.Status, inst.LocalPort,
		inst.SSHHost, inst.SSHPort, inst.TunnelPID, inst.HourlyRate, inst.ID,
	)
	return err
}
...
func (r *SQLiteInstanceRepository) scanMany(query string, args ...any) ([]*entity.Instance, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&inst.HourlyRate, &inst.CreatedAt,
		)
		if err != nil {
			return nil, err
		}
