...
**Geography is a filter, not a hope.** The offer search sorts purely by price, and the cheapest machines cluster in whichever region is currently cheapest — which is how three consecutive deploys landed in Sichuan, whose route to HuggingFace ran at ~1 MB/s, and three more landed on US hosts that could not pull the container image at all while a Romanian host pulled it in seconds. Two filters address this from different angles: `inet_down >= 400 Mbit/s` in every search (the host's own link), and `init --country XX,YY` when the operator knows a region is misbehaving. The country filter maps to vast.ai's `geolocation` field, which matches on the bare two-letter code even though it reads "Romania, RO" — both `{"eq":"RO"}` and `{"in":["RO","DE"]}` were verified against the live API. Codes are validated before the request, because vast.ai answers an unknown code with an empty result set that is indistinguishable from "your other filters are too tight".
