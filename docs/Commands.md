...
| `--sync-folder <path>` | `./COMFY_SYNC` | Where the instance's files are kept locally. Resolved to an absolute path and stored on the instance, so a later `tunnel` or `start` from a different directory syncs to the same place. Ignored by engines that write nothing. |
